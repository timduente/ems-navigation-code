...
public:
	...
	//only for one Channel (0);
	uint8_t increment();
	uint8_t decrement();

	//Increment and Decrement
	//only for one Channel (0) with steps and stepDelay between two Operations
	uint8_t decrement(int steps, int stepDelay);
	uint8_t increment(int steps, int stepDelay);
...
protected:
	...
	//for MCP41HVX1 Increment:
	const static uint8_t kCMD_INCREMENT = B00000100;
	const static uint8_t kCMD_DECREMENT = B00001000;
	...
	void internalSetWiperPosition(byte wiperAddress, unsigned int position,
			bool isNonVolatile);
