...
unsigned int McpDigitalPot::resistanceToPosition(float resistance) {
	...
	return (unsigned int) ((resistance - rW_ohms) / wiperPositionIncrement())
			+ 0.5;
}
...
void McpDigitalPot::internalSetWiperPosition(byte wiperAddress,
		unsigned int position, bool isNonVolatile) {
	byte cmd_byte = 0x00;
	byte data_byte = 0x00;
...
unsigned int McpDigitalPot::getPosition(unsigned int wiperIndex) {
	if (wiperIndex == 1) {
		...
		return (unsigned int) (0x01FF //vorher 01FF
				& this->spiRead(kADR_WIPER0 | kADR_VOLATILE));
	}
}
...
void McpDigitalPot::setPosition(unsigned int wiperIndex,
...
void McpDigitalPot::writePosition(unsigned int wiperIndex,
		unsigned int position) {
	if (wiperIndex == 1) {
...
uint8_t McpDigitalPot::increment() {
	::digitalWrite(slave_select_pin, LOW);
	byte high_byte = SPI.transfer(kCMD_INCREMENT);
	::digitalWrite(slave_select_pin, HIGH);
	return high_byte;
}

uint8_t McpDigitalPot::decrement() {
	::digitalWrite(slave_select_pin, LOW);
	byte high_byte = SPI.transfer(kCMD_DECREMENT);
	::digitalWrite(slave_select_pin, HIGH);
	return high_byte;
}

uint8_t McpDigitalPot::increment(int steps, int stepDelay) {
	::digitalWrite(slave_select_pin, LOW);
	byte high_byte = 0;
	for (int i = 0; i < steps; i++) {
		high_byte = SPI.transfer(kCMD_INCREMENT);
		if(stepDelay>0)
		delay(stepDelay);
	}
	::digitalWrite(slave_select_pin, HIGH);
...
uint8_t McpDigitalPot::decrement(int steps, int stepDelay) {
	::digitalWrite(slave_select_pin, LOW);
	byte high_byte = 0;
	for (int i = 0; i < steps; i++) {
		high_byte = SPI.transfer(kCMD_DECREMENT);
		if(stepDelay>0)
		delay(stepDelay);
	}
	::digitalWrite(slave_select_pin, HIGH);
